  ...
  try {
    ...
    users.forEach((user) => {
      ...
      if (designationData.has(key)) {
        ...
        data.memtypeCounts[user.memtype] =
          (data.memtypeCounts[user.memtype] || 0) + 1
      } else {
    ...
    const bulkOps = Array.from(designationData.entries()).map(
      ([key, data]) => ({
        updateOne: {
          filter: {
    ...
    res
      .status(200)
      .send('Monthly designation counts and total amounts updated successfully')
  } catch (err) {
...
const getMonthlyDesignationCounts = async (req, res) => {
  try {
    ...
    const counts = await DesignationCount.find({
