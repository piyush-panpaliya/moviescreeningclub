	...
	try {
		...
		if (!payment) {
			...
			if (currentDate.isAfter(validityDate)) {
				...
				const formattedDate = `${expirationDate.getDate()}/${
					expirationDate.getMonth() + 1
				}/${expirationDate.getFullYear()}`
				const formattedTime = `${String(expirationDate.getHours()).padStart(
					2,
					'0'
...
exports.saveTempPayment = async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ message: 'An error occurred while saving information' })
	}
}
...
exports.getMembershipData = async (req, res) => {
	try {
		...
		res
			.status(500)
			.json({ message: 'An error occurred while fetching membership data' })
	}
}
...
exports.confirmMembership = async (req, res) => {
	...
	try {
		...
		res
			.status(500)
			.json({ message: 'An error occurred while confirming membership' })
	}
}
